...
#define immB() do { *imm = (SEXT(BITS(i, 31, 31), 1) << 12) | (SEXT(BITS(i, 7, 7), 1) << 11) | \
(SEXT(BITS(i, 30, 25), 6) << 5) | (SEXT(BITS(i, 11, 8), 4) << 1); } while(0)
...
static void decode_operand(Decode *s, int *rd, word_t *src1, word_t *src2, word_t *imm, int type) {
  ...
  *rd     = BITS(i, 11, 7);
  ...
  Log("Address: %u, src1: %u, src2: %u, imm:%u", s->pc, *src1, *src2, *imm);
  if(type == TYPE_B) {
