...
#define ftrace_write(...)                                                      \
  do {                                                                         \
    if (ftrace_log == NULL) {                                                  \
      ftrace_log =                                                             \
          fopen("/home/ashdr/code/ics2023/nemu/build/ftrace-log.txt", "w");    \
    }                                                                          \
...
static void display_elf_header(Elf32_Ehdr eh) {
  ...
  case ELFDATA2MSB:
    ...
    break;
  default:
    ftrace_write("Invalid data encoding\n");
    break;
  }
  ftrace_write("OS/ABI\t=");
  ...
  switch (eh.e_type) {
  case ET_NONE:
    ftrace_write("No file type\n");
    break;
  case ET_REL:
    ftrace_write("Relocatable file\n");
    break;
  case ET_EXEC:
    ftrace_write("Executable file\n");
    break;
  case ET_DYN:
    ftrace_write("Shared object file\n");
    break;
  case ET_CORE:
    ftrace_write("Core file\n");
    break;
  default:
    ftrace_write("Other\n");
    break;
...
static void read_section_header(int fd, Elf32_Ehdr ehdr,
                                Elf32_Shdr shdr_tbl[]) {
  assert(lseek(fd, ehdr.e_shoff, SEEK_SET) == ehdr.e_shoff);
  for (int i = 0; i < ehdr.e_shnum; ++i) {
    assert(read(fd, (void *)&shdr_tbl[i], ehdr.e_shentsize) ==
           ehdr.e_shentsize);
  }
  // 把每个section table entry都读到内存中
}

static void display_section_header(int fd, Elf32_Ehdr ehdr,
                                   Elf32_Shdr shdr_tbl[]) {
  char sh_str
      [shdr_tbl[ehdr.e_shstrndx]
           .sh_size]; // 使用一个大小为section name
                      // table大小的字节数组，即里面存放的是所有section的名字
...
static void read_symbol_table(int fd, Elf32_Ehdr ehdr, Elf32_Shdr shdr_tbl[],
                              int symbol_idx) {
  ...
  for (int i = 0; i < sym_cnt; i++) {
    if (ELF32_ST_TYPE(sym_tbl[i].st_info) == STT_SECTION ||
        ELF32_ST_TYPE(sym_tbl[i].st_info) == STT_NOTYPE) {
      continue;
    }
    ftrace_write(
        " %-3d    %08x %-4d %-10d %s\n", i, sym_tbl[i].st_value,
        ELF32_ST_TYPE(sym_tbl[i].st_info), sym_tbl[i].st_size,
        str_tab +
            sym_tbl[i].st_name); // st_name is the index of symbol in strtab
  ...
  for (int i = 0; i < sym_cnt; i++) {
    if (ELF32_ST_TYPE(sym_tbl[i].st_info) == STT_FUNC) {
      ...
      strncpy(g_symbol_table[g_symbol_tbl_size].name,
              str_tab + sym_tbl[i].st_name, 31);
      g_symbol_tbl_size++;
    }
  }
}
...
static int get_func_idx(paddr_t target) {
  for (int i = 0; i < g_symbol_tbl_size; i++) {
    if (g_symbol_table[i].addr <= target &&
        target < g_symbol_table[i].addr + g_symbol_table[i].size) {
      return i;
    }
  }
  return -1;
...
void trace_func_call(paddr_t pc, paddr_t target) {
  ...
  ftrace_write(FMT_PADDR ": %*scall[%s@" FMT_PADDR "]\n", pc,
               (call_depth - 3) * 2, "",
               (funcid == -1) ? "???" : g_symbol_table[funcid].name, target);
}
...
void trace_func_ret(paddr_t pc) {
  ...
  ftrace_write(FMT_PADDR ": %*sret[%s]\n", pc, (call_depth - 3) * 2, "",
               (funcid == -1) ? "???" : g_symbol_table[funcid].name);
  --call_depth;
}

// 不匹配的函数调用和返回是因为函数调用尾部优化
// 1.elf的section header
// 指明了字符串表在elf文件中的那个位置 2.通过偏移量找到字符串表的位置 3.字符串表将symbol
// table中的标识符的字符串存储了起来 4.根据symbol
// table中的Size属性找到字符串表中的字符串 symbol
// table中还有symbol的虚拟地址、大小、type等信息。 使用地址通过symbol
// table找到字符串表中的字符串的size，再在字符串表中找到字符串