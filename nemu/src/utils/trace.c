...
static void read_symbol_table(int fd, Elf32_Ehdr ehdr, Elf32_Shdr shdr_tbl[], int symbol_idx) {
    ...
    for (int i = 0; i < sym_cnt; i++) {
        if(ELF32_ST_TYPE(sym_tbl[i].st_info) == STT_SECTION || ELF32_ST_TYPE(sym_tbl[i].st_info) == STT_NOTYPE) {
            continue;
        }
...
void trace_func_call(paddr_t pc, paddr_t target) {
    ...
    ftrace_write(FMT_PADDR ": %*scall[%s@" FMT_PADDR "]\n", pc, (call_depth - 3) * 2, "", (funcid == -1) ? "???" : g_symbol_table[funcid].name, target);
}

void trace_func_ret(paddr_t pc) {
    if(call_depth <= 2) return ;
    int funcid = get_func_idx(pc);
    ftrace_write(FMT_PADDR ": %*sret[%s]\n", pc, (call_depth - 3) * 2, "", (funcid == -1) ? "???" : g_symbol_table[funcid].name);
    --call_depth;
}
