  ...
  while (e[position] != '\0') {
    ...
    for (i = 0; i < NR_REGEX; i++) {
      if (regexec(&re[i], e + position, 1, &pmatch, 0) == 0 &&
         ...
         */
        if (rules[i].token_type == TK_NOTYPE || rules[i].token_type == TK_SIGN)
          break;
        ...
        default: {
          ...
          Assert(substr_len < 32,
                 "Length of each token should be smaller or equal than 32");
...
bool is_arithmetic(int tp) {
  if (tp == TK_NUM || tp == TK_LBR || tp == TK_RBR || tp == TK_HEX)
    return false;
  return true;
}
...
int get_level(int tp) {
  if (tp == TK_EQ)
    return 0;
  if (tp == TK_PLUS || tp == TK_SUB)
    return 1;
  if (tp == TK_MUL || tp == TK_DIV)
    return 2;
  return 3; // TK_DREF
}
  ...
  if (p > q) {
    ...
    *ok = false;
    ...
    case TK_DIV: {
      ...
      return val1 /
             val2; // e.g (1 - 2) / 2 should be 0 not 2147483647, because we
                   // generate test case and the result of test case if 0
    } break;
    case TK_DREF: {
      return vaddr_read(val2, 4); // get uint32_t from ram
    }
    default:
      panic("Wrong major operator at range(%d, %d), mpos is %d", p, q, mpos);
    }
  }
}
word_t expr(char *e, bool *success) {
  ...
  for (int i = 0; i < nr_token; i++) {
    if (tokens[i].type == TK_MUL &&
        (i == 0 || is_arithmetic(tokens[i - 1].type))) {
